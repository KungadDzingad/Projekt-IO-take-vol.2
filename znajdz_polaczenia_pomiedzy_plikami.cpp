...
void Graf::znajdz_polaczenia_pomiedzy_plikami()
{
	...
	//w tej pêtli otwieramy wszystkie pliki po koleii do ostatniego
	for (int i = 0; i < pliki.size(); i++)
	{
		...
		{
			//warunek jest spe³niony jeœli znajdzie include w linii 
			if (linia.find("include \"") != string::npos)
			{
				//Wymazanie cudzys³owie na koñcu includa
				linia.erase(linia.size() - 1, linia.size());
				//Wymazanie pierwszych 10 znakow (include i ")  
				linia.erase(0, 10);
				//przypisanie zawartoœci include do odpowiedniego wektora wewn¹trz struktury
